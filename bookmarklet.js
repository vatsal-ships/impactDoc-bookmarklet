(function() {
    ...
    function initializeGapiClient() {
        gapi.client.init({
            ...
            showStatus('Failed to initialize Google APIs', true);
        });
    }
    ...
    function handleAuthClick() {
        tokenClient.callback = async (resp) => {
            ...
            if (masterDocId) {
                // We have a saved document, go straight to content
                contentSection.style.display = 'block';
                showStatus('Successfully authenticated! Ready to add entries.');
            } else {
                // No saved document, ask for document ID
    ...
    async function findOrCreateMonthSection(monthIndex) {
        ...
        try {
            ...
            // Look for existing month section
            for (let i = 0; i < content.length; i++) {
                if (content[i].paragraph && content[i].paragraph.elements) {
                    ...
                    if (text.includes(monthHeader)) {
                        monthExists = true;
                        // Find where to insert entries (after the month header and separator)
            ...
            if (!monthExists) {
                ...
                // Update insert index to after the new section
